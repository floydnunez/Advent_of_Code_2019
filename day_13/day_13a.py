...
while True:
    coord_x, data, index, relative_base = process(data, [], index, relative_base)
    if index == -1:
        break
    coord_y, data, index, relative_base = process(data, [], index, relative_base)
    tile, data, index, relative_base = process(data, [], index, relative_base)
    print("counter[", counter, "] =", coord_x, coord_y, tile)
    counter += 1
    hashval = coord_y * x_res + coord_x
    if hashval in screen:
        print("repeated tile!")
    if tile == 2:
        screen[hashval] = tile
